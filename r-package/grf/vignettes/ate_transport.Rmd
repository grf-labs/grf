...
This vignette gives a brief overview of how to use GRF to estimate average treatment effects (ATEs) on a new target population defined by some covariates $X_{test}$.
...
### Estimator
Some definitions first. $\hat \tau^{(-i)}(X_i)$ are estimates of the conditional average treatment effects $E[Y(1) - Y(0) | X = x]$. $\hat p^{(-i)}(X_i)$ are estimates of the conditional trial-inclusion probabilities $P[S = 1 | X = x]$. $\hat e^{(-i)}(X_i)$ are estimates of the treatment propensities $E[W_i | X = x]$ and $\mu^{(-i)}(X_i, W_i)$  are estimates of the conditional responses $E[Y_i | W_i, X = x]$. The superscript (-1) indicates cross-fitting, i.e. the estimate for unit $i$ is obtained without using unit $i$ for estimation. $n_{RCT}$ is the number of units participating in the trial, and $n_{obs}$ is the number of units in the target population.
...
& \hat \tau_{target} =
  ...
  \frac{1}{n_{obs}}
  \sum_{i \in RCT} \left[
  \frac{1 - \hat p^{(-i)}(X_i)}{\hat p^{(-i)}(X_i)}
  \left(\frac{W_i}{\hat e^{(-i)}(X_i)} - \frac{1-W_i}{1-\hat e^{(-i)}(X_i)}\right)
  \left(Y_i - \hat \mu^{(-i)}(X_i, W_i) \right) \right].
\end{split}
\end{equation}
...
average_treatment_effect_test <- function(forest, X.test, p.hat = NULL) {
  ...
  IO.weights <- (1 - p.hat) / p.hat *
    (W.orig / W.hat - (1 - W.orig) / (1 - W.hat))
  Y.residual <- Y.orig - (Y.hat + tau.hat.train * (W.orig - W.hat))

  tau.hat <- mean(tau.hat.test) + sum(IO.weights * Y.residual) / n.obs
  sigma2.hat <- var(tau.hat.test) / n.obs +
    sum(Y.residual^2 * (IO.weights / n.obs)^2)
...
sprintf("True estimate: %1.2f", mean(X.test[, 1] + 1))
```
