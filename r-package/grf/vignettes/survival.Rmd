...
We also make a third and final "convenience" assumption, that is strictly not required for identification, but which helps guarantee stability of our estimator. We already hinted that estimating the restricted mean involves dividing by censoring probabilities, if these get very low, explosive weights may cause very high variance estimates. In other words, the tail of the observed event density should not be too "thin". If we look at the untruncated density plot in the previous section, we can see that after 10 days there is still some chance of observing an event (the blue shaded region still has some mass). This final (stability) assumption is:
