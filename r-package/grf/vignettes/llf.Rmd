...
LLF predictions with all p variables will be very slow, as will automatic tuning with this scale of data. However, we can still use linear corrections, just with more careful parameters. Users can increase the number of trees for better performance from both methods. The following code is set to not run currently; users can expect it to take approximately 5-6 minutes. 
...
ptm <- proc.time()
preds.llf <- predict(forest, X.test, 
                    linear.correction.variables = selected, 
