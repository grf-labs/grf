...
\begin{tabular}{| c | c | c |}
\hline
Parameter & Value options & Details \\
\hline
Training parameters \\ 
ll.weight.penalty | non-negative double \\
enable.ll.split \\
ll.split.weight.penalty \\
ll.split.lambda \\
ll.split.variables | Covariate index \\
ll.split.cutoff \\
Prediction parameters \\ 
\hline
ll.lambda | non-negative double \\
linear.correction.variables | Covariate index  \\
\end{tabular}
\end{table}
...
LLF predictions with all p variables will be very slow, as will automatic tuning with this scale of data. However, we can still use linear corrections, just with more careful parameters. Users can increase the number of trees for better performance from both methods.
