...
$$
IPW (the terms in parenthesis) accounts for the fact that the prescribed policy $\hat \pi_B(X_i)$ might not match the observed treatment $W_i$ for unit i. The empirical *Qini curve* $\hat Q(B)$ traces out the above value as we vary the available budget $B$, yielding an estimate of $E[\pi_B(X_i) \tau(X_i)] = E[\pi_B(X_i) (Y_i(1) - Y_i(0))]$ - the gain we can achieve when assigning treatment in accordance with our *estimated* CATEs, and costs, at various values of the budget B.
...
cost <- 1
# The maximum spend per unit to fit the curve on.
max.budget <- 1
# Use the maq package to fit a Qini curve, using 200 bootstrap replicates for SEs.
qini <- maq(tau.hat, cost, max.budget, IPW.scores, R = 200)

# Form a baseline Qini curve that uses the ATE rather than the CATE to assign treatment.
qini.baseline <- maq(tau.hat, cost, max.budget, IPW.scores, target.with.covariates = FALSE)
...
cost <- 1
max.budget <- 1
ex.qini <- maq(cate.hat.test, cost, max.budget, DR.test, R = 200)
qini.ate <- maq(cate.hat.test, cost, max.budget, DR.test, target.with.covariates = FALSE)

par(mfrow = c(1, 2))
plot(rate, sub = "", xlab = "Treated fraction", main = "TOC curve")

plot(ex.qini, ylab = "", xlab = "Spend", main = "Qini curve", xlim = c(0, 1))
plot(qini.ate, add = TRUE, lty = 2)
```

The TOC quantifies an average treatment effect over the quantiles of some scoring rule. The Qini curve quantifies an average treatment effect for units predicted to benefit from treatment (that is, the predictions $\hat \tau(X_i)$ are positive) as we vary a budget constraint (thus taking an optimization approach for evaluation).
...
IPW.scores <- Y.ipw[test, -1] - Y.ipw[test, 1]

# Note: "maq" has a convenience function that does the above via `get_ipw_scores(Y[test], W[test])`.

# The cost of arm 1 and 2
cost <- cbind(X[test, 4] / 4, X[test, 5])
max.budget <- 1

ma.qini <- maq(tau.hat, cost, max.budget, IPW.scores, R = 200)
plot(ma.qini)
```

We can also compute single Qini curves for each treatment arm and overlay them in the plot:
```{r}
qini.arm1 <- maq(tau.hat[, 1], cost[, 1], max.budget, IPW.scores[, 1], R = 200)
qini.arm2 <- maq(tau.hat[, 2], cost[, 2], max.budget, IPW.scores[, 2], R = 200)
...
```{r}
ma.qini.basline <- maq(tau.hat, cost, max.budget, IPW.scores, target.with.covariates = FALSE, R = 200)
