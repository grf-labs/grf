...
The Qini curve for a multi-armed policy looks like
```{r}
plot(ma.qini)
```
...
The multi-armed Qini generalization allows us to answer questions such as "For a specific spend level, what is the estimated increase in gain when optimally targeting with both arms as opposed to using only a single arm?" For example, at $B = 0.3$ this difference over arm 1 (illustrated by the orange vertical line in the above plot) would be would be
```{r}
...
We can retrieve the underlying estimated policy through `predict`, for example at $B=0.3$
```{r}
...
```{r}
difference_gain(ma.qini, ma.qini.basline, spend = 0.3)
```
