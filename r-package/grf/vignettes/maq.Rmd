...
$$
IPW (the terms in parenthesis) accounts for the fact that the prescribed policy $\hat \pi_B(X_i)$ might not match the observed treatment $W_i$ for unit i. The empirical *Qini curve* $\hat Q(B)$ traces out the above value as we vary the available budget $B$, yielding an estimate of $E[\pi_B(X_i) \tau(X_i)]$ - the gain we can achieve when assigning treatment in accordance with our *estimated* CATEs, and costs, at various values of the budget B.
