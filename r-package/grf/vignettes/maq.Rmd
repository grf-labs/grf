...
par(mfrow = c(1, 2))
plot(rate, sub = "", xlab = "Treated fraction", main = "TOC")

plot(qini, ylab = "", xlab = "Spend", main = "Qini")
...
The [maq](https://github.com/grf-labs/maq) package performs this exercise efficiently, and we’ll here jump straight into a toy example with 2 treatment arms and a control. We are simulating a simple example where one treatment arm (number 2) is more effective but costlier:
```{r}
...
# The cost of arm B and C (these can also vary by unit, in which case we can supply a matrix).
cost <- c(1/4, 2)
max.budget <- 2
...
The multi-armed Qini generalization allows us to answer questions such as "For a specific spend level, what is the estimated increase in gain when optimally targeting with both arms as opposed to using only a single arm?" For example, at $B = 0.5$ this difference over arm 1 would be would be
```{r}
difference_gain(ma.qini, qini.arm1, spend = 0.5)
```

We can retrieve the underlying estimated multi-armed policy through `predict`, for example at $B=0.5$:
```{r}
pi.hat <- predict(ma.qini, spend = 0.5)
head(pi.hat)
```
...
colMeans(pi.hat)
```
i.e., at this spend level assigning `r mean(pi.hat[,1])` % of the units to arm 1 and `r mean(pi.hat[,2])` % of the units to arm 2 is optimal.
...
```{r}
difference_gain(ma.qini, ma.qini.basline, spend = 0.5)
```
...
\end{aligned}
$$
Below is how one could solve for this policy using a generic solver like `lpSolve`.
```{r}
library(lpSolve)

# Encode the problem according to lpSolve.
K <- ncol(tau.hat)
x.coeffs <- c(t(tau.hat)) / nrow(tau.hat)
A.mat <- matrix(0, nrow(tau.hat), length(x.coeffs))
start <- 1
...
costs <- matrix(cost, nrow(tau.hat), ncol(tau.hat), byrow = TRUE)
c.coeff <- c(t(costs)) / nrow(costs)
f.con <- rbind(A.mat, c.coeff)
f.dir <- rep("<=", nrow(f.con))

# Solve it for a specific budget B.
B <- 0.5
f.rhs <- c(rep(1, nrow(A.mat)), B)
lp.res <- lp("max", x.coeffs, f.con, f.dir, f.rhs)

pi.mat.lp <- matrix(lp.res$solution, nrow = nrow(tau.hat), byrow = TRUE)

# A single point on the curve:
sum(pi.mat.lp * Y.ipw.test) / nrow(tau.hat)

# maq has already computed the entire path, and deliver the estimated value at B with:
average_gain(ma.qini, spend = B)
```

Note that manually estimating the policy $\hat \pi_B$ involves repeatedly solving `lp` with different budget constraints, `f.rhs`, one for each point on the spend curve. This would quickly get computationally infeasible, particularly for large $n$. `maq` computes the entire *solution path* to the above linear program for all $B$ constraints, for details see Algorithm 2 in the [paper](https://arxiv.org/abs/2306.11979).
