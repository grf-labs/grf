...
## A grf overview
This section gives a lighting tour of some of the conceptual ideas behind GRF in the from of a walk through of *Causal Forests*. It starts with describing how the predictive capabilities of the modern machine learning toolbox can be leveraged to non-parametrically control for counfounding when estimating average treatment effects, and how Breiman's (2001) random forest can be re-purposed as an adaptive nearest neighbor finder to detect treatment effect heterogeneity.
...
Assumption 1) is an "identifying" assumption we have to live with, however 2) and 3) are modelling assumptions that we can question. Let's start with assumption 2): this is a strong parametric modelling assumption that requires the counfounders to have a linear effect on the outcome, and that we should be able to relax by relying on semi-parametric statistics.
...
So, to recap so far: we have a way to adopt the modern ML toolkit to *non-parametrically control for counfounding* when estimating an average treatment effect, and still retain desirable statistical properties such as unbiasdness and consistency.
...
1) Building phase: Build $B$ trees which greedily places covariate splits that maximizes the squared difference in subgroup means $n_L n_R (\bar y_L - \bar y_R)^2$.
...
where we have defined $\color{blue}{\alpha_i(x)}$ as the frequency with which the $i$-th training sample falls into the same leaf as $x$. I.e, random forest essentially predicts a weighted average of outcomes. These weights are the so-called random forest adaptive neighbourhood weights, and they capture how "similar" a target sample $x$ is to each of the training samples $X_i$, $i = 1 \ldots n$. Zeileis, Hothorn & Hornik (2003).
...
The TOC is informative here since it appear to be indicating a lower quantile $q$ where there may be a decline in cognition. Farbmacher et al. (2021) indicates that there appears to be a small subset of the population experiencing adverse effects from poverty, and this is a setting where the AUTOC can be more more powerful than the so-called Qini coefficient in detecting heterogeneity. Computing the same evaluations using the Qini weighting yields lower estimates.
