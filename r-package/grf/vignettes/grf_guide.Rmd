...
The data contains 2 480 observations of 27 covariates. Fitting at CATE estimator on such a large amount of covariates relative to the sample size is likely to be underpowered, so below we first predict the conditional mean $E[Y_i | X_i]$, then use a simple forest variable importance to choose which covariates to keep in the CATE estimation step.
```{r}
...
# Keep the following variables for CATE estimation
keep <- colnames(X)[which(varimp.Y > median(varimp.Y))]
keep
```
