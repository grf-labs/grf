...
## A grf overview
This section gives a lighting tour of some of the conceptual ideas behind GRF in the form of a walk through of *Causal Forests*. It starts with describing how the predictive capabilities of the modern machine learning toolbox can be leveraged to non-parametrically control for confounding when estimating average treatment effects, and how Breiman's (2001) random forest can be repurposed as an adaptive nearest neighbor finder to detect treatment effect heterogeneity. (For a great overview of the fundamental tension between prediction and estimation, see the essay by Efron, 2020).

### Machine learning for causal inference
Imagine we observe outcomes $Y_i$ along with a binary treatment indicator $W=\{0, 1\}$ and are interested in measuring the average causal effect of the intervention on the outcome. If we are not in a randomized control trial setting, but have collected a set of covariates $X_i$ which we believe could plausible account for confounding, then we could run a regression of the type (Imbens & Rubin, 2005)
...
**Non-constant treatment effects**. What if we wish to relax assumption 3)? We could a-priory specify certain subgroups and run separate regressions for each subgroup and obtain different estimates of $\tau$. Ideally How can we take a more data-adaptive approach? We can continue with Robinson's partially linear model from above, and instead just posit:

$$
Y_i = \color{red}{\tau(X_i)}W_i + f(X_i) + \varepsilon_i, \, \, E[\varepsilon_i | X_i, W_i] = 0,
$$
where $\color{red}{\tau(X)}$ is the conditional average treatment effect $E[Y(1) - Y(0) | X_i = x]$. How do we estimate this? If we imagine we had access to some neighborhood $\mathcal{N}(x)$ where $\tau$ was constant, we could proceed exactly as before, by doing a residual-on-residual regression on the samples belonging to \mathcal{N}(x), i.e.:
...
### Random forests as adaptive neighborhood finder

Breiman's random forest for predicting the conditional mean $\mu(x) = E[Y_i | X_i = x]$ can be briefly summarized in two steps:

1) Building phase: Build $B$ trees which greedily places covariate splits that maximizes the squared difference in subgroups means $n_L n_R (\bar y_L - \bar y_R)^2$.
2) Prediction phase: Aggregate each tree's prediction to form final point estimate by averaging the outcomes $Y_i$ that fall into the same terminal leaf $L_b(X_i)$ as the targets sample $x$: $\hat \mu(x) = \frac{1}{B} \sum_{b=1}^{B} \sum_{i=1}^{n} \frac{Y_i 1\{Xi \in L_b(x)\}} {|L_b(x)|}$
...
For a single tree and single split, Figure 1) shows the optimal split point in phase 1): it's the vertical line that makes the left and right means of $Y_i$ as similar as possible (or equivalently, the difference in means as large as possible). An intuition for why slicing up the covariate space into many rectangles is a good approach, is that in high dimensions these rectangles might be "narrow" in directions which don't matter, but wide in those which does (unlike a typical k-NN "circular" neighborhood that needs to grow in radius).

Again, for a single tree with one split, Figure 1) shows a target sample $x$ we wish to make a prediction for as a cross in the right hand leaf. Phase 2) would average the $Y_i$'s on this side, then repeat the same exercise for the rest of the $B$ trees, then average these to produce a final prediction. The expression for $\hat \mu(x)$ is a double sum, so we can swap the order of summation and write
...
where we have defined $\color{blue}{\alpha_i(x)}$ as the frequency with which the $i$-th training sample falls into the same leaf as $x$. I.e, random forest essentially predicts a weighted average of outcomes. These weights are the so-called random forest adaptive neighborhood weights, and they capture how "similar" a target sample $x$ is to each of the training samples $X_i$, $i = 1 \ldots n$. Zeileis, Hothorn & Hornik (2003).

**Causal Forest** essentially combines Breiman (2001) and Robinson (1988) by:

1) Building phase: Greedily places covariate splits that maximizes the squared difference in subgroup treatment effects $n_L n_R (\hat \tau_L - \hat \tau_R)^2$, where $\hat \tau$ is obtained by running Robinson's residual-on-residual regression for each possible split point[^1].
2) Use the resulting forest weights $\color{blue}{\alpha_i(x)}$ to estimate


$$
\tau(x) := lm\Biggl( Y_i - \hat m^{(-i)}(X_i) \sim W_i - \hat e^{(-i)}(X_i), \text{weights} = \color{blue}{\alpha_i(x)} \Biggr).
$$
That is, *Causal Forest* is running a "forest"-localized version of Robinson's regression[^2]. This adaptive-weighting (instead of leaf-averaging) coupled with some other forest construction details known as "honesty" and "subsampling" can be used to give asymptotic guarantees for estimation and inference with random forests (Wager & Athey, 2018). The arguments `Y.hat` and `W.hat` supply estimates of $m(x)$ and $e(x)$.

The procedure we outlined above can be generalized (thus the name *generalized* random forest) to estimate other quantities beside heterogeneous treatment effects. The key ingredient is phase 1) above, by recognizing that Robinson's regression equivalently be expressed on "estimating equation" form given by a particular scoring function $\psi_{\tau}(\cdot)$. `grf` (Athey, Tibshirani & Wager, 2019) generalizes phase 1) to target heterogeneity in some arbitrary parameter given as the solution to this estimating equation. That is, `grf` essentially takes as input a formulation of how to estimate a certain parameter[^3] $\theta$ *without* the presence of covariates (in the form of the scoring function $\psi$) then finds forest weights $\alpha(x)$ that yields *covariate*-specific estimates $\theta(x)$.

### Final ingredient: Gaining efficiency with doubly robust estimation
We briefly mentioned that Robinson's residual-on-residual satisfied an *orthogonality* property in the sense that the target parameter ($\tau$) could be estimated well (in the sense of root-n convergence) despite nuisance components ($m(x)$ and $e(x)$) were estimated at slower rates. This is a desirable property of an estimating equation $\psi$, particularly if we wish to estimate nuisance component machine learning methods that, while more flexible than parametric models, converge at slower rates (Stone, 1980).

For estimating quantities that take the form of an average, such as an average treatment effect (ATE) $\tau = E[Y_i(1) - Y(0)]$, or an average of CATEs for a certain subgroup, there is yet another efficiency gain to obtain by drawing from classical insights from semi-parametric statistics: Robins, Rotnitzky & Zhao (1994) showed that so-called Augmented Inverse Probability Weighted (AIPW) estimator is asymptotically optimal for $\tau$ (meaning that among all non-parametric estimators it has the lowest variance). It looks like the following:
...
$$
where $\mu(X_i, 1),  \mu(X_i, 0)$ are estimates of the conditional means in the two treatment arms: $\mu(X_i, W_i) = E[Y_i | X_i = x, W_i = w]$. This expression can be arranged and expressed as
...
$$
We can identify the 3 terms as: 1) an initial treatment effect estimate ($\tau(X_i)$), 2) a debiasing weight ($\frac{W_i - e(X_i)}{e(X_i)[1 - e(X_i)]}$), and 3) a residual ($Y_i - \mu(X_i, W_i)$). This formulation as a one-step plug-in update (i.e, an initial estimate plus a bias correction) is quite general and extends to other target parameters, such as for example a continuous treatment, but where the debiasing weights would differ. Moreover, when using "cross-fitting" when forming the nuisance estimates, the resulting estimator with plug-in values is still efficient (Zheng & van der Laan 2011, Chernozhukov et al., 2018).

This is the estimation approach taken by `grf` when computing summary measures such as an average treatment effect, and the associated doubly robust scores $\hat \Gamma_i$ can be retrieved by `get_scores`. It is also the estimation approach that underlies summary measures such as the `RATE` and `best linear projection` (as well as doubly robust policy learning). Summary measures built on these play a crucial role in interpreting non-parametric CATE estimates, as they are by the fundamental limits of non-parametric estimation (machine learning included), noisy point estimates. The next two sections walk through empirical examples.
...
A recipe for valid inference with RATE is to split the data into a training and evaluation set, one for evaluating the RATE, and another to train a CATE estimator. The drawback of this approach is that we may detect different findings depending on the random sample used to define the evaluation and training sets. The the RCT was clustered at the school level, we'll draw random units at the school level.
...
plot(rate.cate, main = "TOC: Rank by decreasing estimated CATE")
rate.cate
```

For this data set in particular, whether the RATE is significant or not depends on if the CATE estimator is trained on a subsample with sufficient signal. There are more ways to utilize the RATE metric besides CATE rankings, we can compute the TOC by for example the ranking of some specific variable. The previous section suggested that students with high financial autonomy benefits less from the program. We can use the RATE to evaluate how prioritizing students based on this index does:

```{r}
# We use the trained forest on the full data since are computing the RATE based on a covariate.
rate.fin.index <- rank_average_treatment_effect(
  cf,
  -1 * X$financial.autonomy.index, # Multiply by -1 do order by decreasing index
  subset = !is.na(X$financial.autonomy.index) # Ignore missing X-values
  )
plot(rate.fin.index, xlab = "Treated fraction", ylab = "Increase in test scores", main = "TOC: Rank by increasing financial autonomy")
rate.fin.index
```
The TOC suggest that students with a lower financial autonomy index benefits more than average from the program, and AUTOC is significant at conventional levels: `rate.fin.index$estimate / rate.fin.index$std.err` = `r rate.fin.index$estimate / rate.fin.index$std.err`. Compared to the standard deviation of the outcome `sd(Y)` = `r sd(Y)` the benefit also appears meaningful.
...
Denote a by $\pi(X_i)$ a function (policy) that takes a covariate vector $X_i$ and maps it to a binary decision of whether the subject should be treated or not: $\pi \mapsto \{0, 1\}$ (this can easily be extended to multiple treatments). The task of finding such a policy is often referred to as policy learning. Given doubly robust estimates $\hat \Gamma_i$ of binary treatment effects, `policytree` finds a tree-based policy $\pi(X_i)$ that maximizes
...
Below we'll walk through an example of using the [policy_tree](https://grf-labs.github.io/policytree/reference/policy_tree.html) function from `policytree` to find an optimal depth-2 decision tree policy. This function uses a purpose-built C++ solver to find a shallow and optimal decision rule, and there are some useful suggestions in the [docstring](https://grf-labs.github.io/policytree/reference/policy_tree.html) on how this approach can be expected to scale (for example, reducing the $X_j$ cardinality leads to fewer split points to search over). We are using separate data sets to fit and evaluate the policy:
...
In this section we walk through an example following Farbmacher et al. (2021) who studies a causal forest application using data from Carvalho et al. (2016). Carvalho et al. (2016) conducted an experiment where they randomly assigned low-income individuals to perform a cognitive test before ($W=1$) or after ($W=0$) payday. The outcome is the performance on a test designed to measure cognitive ability. The original study did not find an effect on average, however Farbmacher et al. (2021) conducts a HTE analysis and finds evidence suggesting there are parts of the population where poverty impairs cognitive function. A processed copy of this data is stored on the [github repo](https://github.com/grf-labs/grf/tree/master/r-package/grf/vignettes/carvalho2016.csv) and contains observations from around 2 500 individuals along with 27 characteristics such as age, income, etc. Education is stored as an ordinal value where a high value means high education and a low less education (4: college graduate, 3: some college, 2: high school graduate, 1: less than high school).
...
The outcome is the number of correct answer on a test measuring cognitive ability, so a negative CATE means an impairment in cognition due to poverty, so in computing the RATE (AUTOC) we rank by the most negative CATEs. We also consider age (old to youn), as it shows up high in the split frequency.
```{r}
...
Zheng, Wenjing, and Mark J. Laan. "Cross-validated targeted minimum-loss-based estimation." In Targeted Learning, pp. 459-474. Springer, New York, NY, 2011.
...
[^2]: For a discussion of how Robinson's transformation can be used to motivate a generic loss function for heterogeneous treatment effects estimation (the *R-learner*), see Nie & Wager (2021).
