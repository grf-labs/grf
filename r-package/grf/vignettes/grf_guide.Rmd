...
& = ``\color{blue}{\alpha_i(x)} - weighted \, outcomes'',
\end{split}
\end{equation*}

where we have defined $\color{blue}{\alpha_i(x)}$ as the frequency with which the $i$-th training sample falls into the same leaf as $x$. These weights are the so-called random forest adaptive neighbourhood weights, and they capture how "similar" a target sample $x$ is to each of the training samples $X_i$, $i = 1 \ldots n$.  
