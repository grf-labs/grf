...
## A Causal Forest + grf overview
This section gives a lighting tour of some of the conceptual ideas behind GRF, in the from of a walkthrough of *Causal Forests*. It starts with describing how the predictive capabilities of the modern machine learning toolbox can be leveraged to non-parametrically control for counfounding when estimating average treatment effects, then covers how Breiman's (2001) random forest can be re-purposed as an adaptive nearest neighbor finder to detect treatment effect heterogeneity.
...
### Final statistical ingredient: Gaining efficiency with doubly robust estimation
...
For estimating quantities that take the form of an average, such as an average treatment effect (ATE) $\tau = E[Y_i(1) - Y(0)]$, or an average of CATEs for a certain subgroup, there is yet another efficiency gain to obtain by drawing from classical insights from semi-parametric statistics: Robins et al. (1994) showed that so-called Augmented Inverse Probability Weighted (AIPW) estimator is asymptotically optimal for $\tau$ (meaning that among all non-parametric estimators it has the lowest variance). It looks like the following:
...
Van Der Laan, Mark J., and Daniel Rubin. "Targeted maximum likelihood learning." The international journal of biostatistics 2, no. 1 (2006).
...
[^3]: Note that $\theta$ does not have to be a scalar, we 
