...
## A grf overview
This section gives a lighting tour of some of the ideas behind GRF. It starts with describing how the predictive capabilities of the modern machine learning toolbox can be leveraged to non-parametrically control for counfounding when estimating average treatment effects, then covers how Breiman's (2001) random forest can be re-purposed as an adaptive nearest neighbor finder to detect treatment effect heterogeneity.
...
$$
\tau(x) := lm\Biggl( Y_i - m^{(-i)}(X_i) \sim W_i - e^{(-i)}(X_i), \text{weights} = 1\{X_i \in \mathcal{N}(x) \}\Biggr),
$$
(the superscript $^i$ denotes cross-fitting). This is conceptually what *Causal Forest* does, it estimates the treatment effect $\tau(x)$ for a target sample $X_i = x$ by running a weighted residual-on-residual regression on samples that have similar treatment effects. These *weights* play a crucial role, so how does `grf` find them? The next section quickly recaps Breiman's random forest for regression, then explains how it can be extended to an adaptive neighborhood finder.

### Random forests as adaptive neighborhood finder

Breiman's random forest for predicting the conditonal mean $\mu(x) = E[Y_i | X_i = x]$ can be briefly summarized in two steps:

1) Building phase: Build $B$ trees which greedily places covariate splits that maximizes $n_L n_R (\bar y_L - \bar y_R)^2$.
...
For a single tree and split, phase 1) Figure 1.
...
### Final statistical ingredients: Doubly robust estimates
