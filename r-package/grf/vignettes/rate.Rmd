...
If we as a toy example let $\tau(X_i) = X_1$ where $X_1 \sim N(0, 1)$, then the TOC curve would look like:
...
ggplot(df, aes(x = q, y = TOC)) +
  ...
  annotate(geom = 'text', x = .2, y = ypoint, color = 'black', hjust = -0.1,
           label = 'ATE of top 5% minus ATE') +
  annotate("segment", x = .2, xend = .1, y = ypoint, yend = ypoint,
...
```{r}
horizon <- 3 * 365
csf.sprint <- causal_survival_forest(X.sprint, Y.sprint, W.sprint, D.sprint, num.trees = 100, #TOUPDATE
                                     W.hat = mean(W.sprint), target = "RMST", horizon = horizon)

csf.accord <- causal_survival_forest(X.accord, Y.accord, W.accord, D.accord, num.trees = 100, #TOUPDATE
