...
test_that("quantile forest split frequencies are reasonable", {
	...
	qrf = quantile.forest(X, Y, quantiles = c(0.1, 0.5, 0.9), mtry=p, min.node.size = 10, sample.fraction=0.632)
	split.frequencies = compute_split_frequencies(qrf, 4)
...
test_that("causal forest split frequencies are reasonable", {
  ...
  ccc = causal.forest(X, Y, W, mtry = p, alpha=0.05)
  freq = compute_split_frequencies(ccc, 4)
...
test_that("tree indexes are one-indexed", {
	...
	expect_error(examine.tree(qrf, 0))
	examine.tree(qrf, n)
})
