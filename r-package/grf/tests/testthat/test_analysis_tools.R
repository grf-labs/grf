...
test_that("examining a tree gives reasonable results", {
	...
	qrf = quantile_forest(seed=1, X, Y, quantiles = c(0.1, 0.5, 0.9), mtry=p, min.node.size = 10, sample.fraction=0.632)
...
test_that("leaf samples are indexed correctly", {
	...
	qrf = quantile_forest(seed=1, X, Y, sample.fraction=1.0, honesty=FALSE)
...
test_that("computing sample weights gives reasonable results", {
	...
	rrf = regression_forest(seed=1, X, Y, mtry=p)
