...
test_that("using big ci.group.size doesn't result in tiny CIs", {
  ...
  forest.big.group = regression_forest(X, Y, num.trees = 2000, ci.group.size = 200, seed = seed)
  ...
  forest.sm.group = regression_forest(X, Y, num.trees = 2000, ci.group.size = 10, seed = seed)
...
test_that("instrumental CIs are invariant to scaling Z", {
  ...
  y.forest = regression_forest(X, Y, seed = seed)
  Y.hat = predict(y.forest)$predictions

  forest = causal_forest(X, Y, W, Y.hat = Y.hat, W.hat = 0, seed = seed)
  tau.hat = predict(forest, newdata = X.test, estimate.variance = TRUE)
  error.standardized = (tau.hat$predictions - tau.true) / sqrt(tau.hat$variance.estimates)
  expect_true(mean(abs(error.standardized) > qnorm(0.975)) <= 0.15)
  ...
  forest.iv = instrumental_forest(X, Y, W, Z,
                                  Y.hat = Y.hat, W.hat = 0, Z.hat = 0, seed = seed)
