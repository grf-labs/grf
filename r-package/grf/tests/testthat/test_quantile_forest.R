...
test_that("quantile forests give reasonable results", {
	...
	qrf.gen = quantile.forest(X, Y, quantiles = c(0.1, 0.5, 0.9), mtry=p, min.node.size = 10, sample.fraction=0.632, ci.group.size=1)
	...
	mean_error = mean(abs(preds.truth - preds.gen))
