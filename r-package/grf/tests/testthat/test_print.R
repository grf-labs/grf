...
test_that("basic printing is successful", {
  ...
  q.forest = quantile_forest(seed=1, X, Y, quantiles = c(0.1, 0.5, 0.9), num.trees = 50)
...
test_that("printing a forest with one regressor is successful", {
  ...
  Y = X[,1] * rnorm(n)
  r.forest = regression_forest(seed=1, X, Y)
 ...
 test_that("tuning output printing is successful", {
  ...
  Y = runif(n)
  W = rbinom(n, 1, 0.5)
  tuned.forest = causal_forest(seed=1, X, Y, W, num.trees = 100, tune.parameters = TRUE)
