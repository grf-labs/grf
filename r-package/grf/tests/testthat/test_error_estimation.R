...
test_that("regression error estimates are reasonable", {
	...
	output.5.reps = replicate(20, {
	  forest.5 = regression_forest(seed=1000, X, Y, num.trees = 5, ci.group.size = 1, min.node.size = 1)
	...
	forest.200 = regression_forest(seed=1000, X, Y, num.trees = 200, ci.group.size = 1)
...
test_that("causal error estimates are reasonable", {
  ...
  W.forest = regression_forest(seed=1000, X, W, num.trees = 500, sample.fraction = 0.2)
  W.hat = predict(W.forest)$predictions

  Y.forest = regression_forest(seed=1000, X, Y, num.trees = 500, sample.fraction = 0.2)
  ...
  output.10.reps = replicate(20, {
    cf.10 = causal_forest(seed=1000, X, Y, W, Y.hat = Y.hat, W.hat = W.hat,
  ...
  output.20.reps = replicate(10, {
    cf.20 = causal_forest(seed=1000, X, Y, W, Y.hat = Y.hat, W.hat = W.hat,
  ...
  cf.400 = causal_forest(seed=1000, X, Y, W, Y.hat = Y.hat, W.hat = W.hat,
