...
test_that("a simple survival forest workflow works", {
  ...
  sf <- survival_forest(X, Y, C, num.trees = 50)
...
test_that("sample weighted survival prediction is invariant to weight rescaling", {
  ...
  sf1 <- survival_forest(X, Y, C,
                         sample.weights = sample.weights,
                         num.trees = 50,
                         seed = 1)

  sf2 <- survival_forest(X, Y, C,
...
test_that("survival_forest works as expected with missing values", {
  ...
  sf <- survival_forest(X, Y, C, num.trees = 500)
  ...
  Xr[is.nan(Xr)] <- 1e9
  X.mia <- cbind(Xl, Xr)
  sf.mia <- survival_forest(X.mia, Y, C, num.trees = 500)
