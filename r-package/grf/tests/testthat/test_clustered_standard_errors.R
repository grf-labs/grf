...
test_that("Clustered standard errors are greater than unclustered", {
  ...
  forest_corrected <- regression_forest(seed=1, X_cluster,
                                        Y_cluster,
                                        ci.group.size = 4,
                                        clusters = clusters,
  ...
  forest_no_cluster <- regression_forest(seed=1, X,
                                         Y,
                                         ci.group.size = 4,
                                         samples.per.cluster = 1)
  preds_no_cluster.oob <- predict(forest_no_cluster, estimate.variance = TRUE)

  forest_uncorrected <- regression_forest(seed=1, X_cluster,
                                          Y_cluster,
                                          ci.group.size = 4,
                                          samples.per.cluster = 1)
  preds_uncorrected.oob <- predict(forest_uncorrected, estimate.variance = TRUE)

  forest_corrected_no_clusters <- regression_forest(seed=1, X,
                                                    Y,
                                                    ci.group.size = 4,
