...
test_that("causal forest tuning decreases prediction error", {
    ...
    forest = causal_forest(X, Y, W, num.trees = 400, min.node.size = 1, tune.parameters = FALSE, seed = seed)
    preds = predict(forest)
    error = mean((preds$predictions - TAU)^2)

    tuned.forest = causal_forest(X, Y, W, num.trees = 400, tune.parameters = TRUE, seed = seed)
