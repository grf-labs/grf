...
test_that("output of tune local linear causal forest is consistent with prediction output", {
  ...
  expect_equal(tuning.results$oob.predictions[, 1], pred.ll.min, tolerance = 10^-6)
  ...
  expect_equal(tuning.results$oob.predictions[, length(tuning.results$lambdas)], pred.ll.max, tolerance = 10^-6)
})
