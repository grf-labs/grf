...
custom_forest <- function(X, Y, sample.fraction = 0.5, mtry = NULL, 
    ...
    clusters = NULL, samples_per_cluster = NULL, status) {

    validate_X(X)
    if(length(Y) != nrow(X)) { stop("Y has incorrect length.") }
    if(length(status) != nrow(X)) { stop("status has incorrect length.") }
    ...
    data <- create_data_matrices(X, Y, status)
...
#' @export
predict.custom_forest <- function(object, newdata = NULL, num.threads = NULL, timepoints, ...) {        
    forest.short <- object[-which(names(object) == "X.orig")]

    X <- object[["X.orig"]]
    train.data <- create_data_matrices(X, object[["Y.orig"]], object[["status.orig"]])
    outcome.index <- ncol(X) + 1
    ...
    if (!is.null(newdata)) {
        ...
        custom_predict(forest.short, timepoints, train.data$default, train.data$sparse, outcome.index,
            data$default, data$sparse, num.threads)
    } else {
        custom_predict_oob(forest.short, timepoints, train.data$default, train.data$sparse, outcome.index, num.threads)
    }
}
