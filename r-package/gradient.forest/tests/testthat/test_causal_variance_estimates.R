...
test_that("causal variance estimates are reasonable", {
	...
	forest.causal = instrumental.forest(X, Y, W, W, sample.fraction = 0.1, min.node.size = 5, split.regularization = 0)
	preds.causal = predict(forest.causal, X.test, estimate.variance=TRUE)

	preds.med = preds.causal[[1]]
	preds.low = preds.causal[[1]] - 1.96 * sqrt(preds.causal[[2]])
	preds.high = preds.causal[[1]] + 1.96 * sqrt(preds.causal[[2]])
