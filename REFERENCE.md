...
GRF can only handle numerical inputs. Here are some ways in which you can proceed If your data has a column with categorical values.

If the categorical column has a natural ordering, then simply encode its levels as integers according to this ordering. For instance, an individual's educational attainment could be represented with 1 for "primary" 2 for "secondary", 3 for "undergraduate", and so on. This ordering will make it easy for forests to split individuals into lower and higher education groups. Moreover, even if your variable does not have an obvious ordering at first sight, you may want to be a little creative in imposing an artificial ordering that is relevant to your statistical problem. For example, numbering contiguous geographical areas sequentially can often help forests make splits that group nearby areas together. If geographical proximity is important, then the forest performance will likely increase.

However, if there is no easy way to order the categories, then one common option is to create one-hot encoded vectors, also know as dummy vectors. This approach replaces the single categorical column with several binary columns. Unfortunately, this sort of representation can be wasteful. That is because each binary column will likely not carry a lot of information, as it only encodes whether or not a variable belongs to a particular level. In case your categorical column has many levels, you might want to use a more sophisticated representation method, such as feature hashing. Alternatively, we recommend users try our sister package [`sufrep`](https://github.com/grf-labs/sufrep), which implements several categorical variable representation methods. It is currently in beta version, so please feel free to submit an issue for bugs or additional feature requests.

Finally, please bear in mind that the statistical performance of any statistical algorithm will depend on how the data is represented. Therefore, it may be worthwhile to spend some time thinking about how to encode your variables before the actual analysis.
