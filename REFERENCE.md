...
- If `estimate.variance` is enabled, sample half of the clusters. Each 'tree group' is associated with this half-sample of clusters (as described in the section on the 'Variance Estimates' section). If we are not computing variance estimates, then keep the full sample instead.
