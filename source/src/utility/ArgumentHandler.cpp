...
void ArgumentHandler::checkArguments() {
  ...
  if (treetype == TREE_SURVIVAL && impmeasure == IMP_GINI) {
    throw std::runtime_error("Node impurity variable importance not supported for survival forests. See '--help' for details.");
  }
...
void ArgumentHandler::displayHelp() {
  ...
  std::cout << "    " << "                              TYPE = 2: Permutation importance, scaled by standard errors."
      << std::endl;
  std::cout << "    " << "                              TYPE = 3: Permutation importance, no scaling."
      << std::endl;
