...
qrf.gen = quantile.forest(X, Y, quantiles = c(0.1, 0.5, 0.9), mtry=p, min.node.size = 10, sample.fraction=0.632, num.trees=500)
preds.gen = predict(qrf.gen, X.test.df, quantiles = c(0.1, 0.5, 0.9))
...
plot(NA, NA, xlim = c(-1, 1), ylim = range(preds.meinshausen, preds.gen, preds.truth), xlab = "X", ylab = "Y")
...
lines(X.test[,1], preds.gen[,1], col = 2, lwd = 2, lty = 2)
lines(X.test[,1], preds.gen[,2], col = 2, lwd = 2, lty = 1)
lines(X.test[,1], preds.gen[,3], col = 2, lwd = 2, lty = 2)
