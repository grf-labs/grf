...
### Working with the R package in RStudio (alternative to using `build_package.R`)
Create symlinks to the C++ source by executing `r-package/grf/src/create_symlinks.sh` (for Windows users we leave this step to you). Provided you have
the necessary compiler in place, the default `grf.Rproj` file will provide all of Rstudio's build functionality. The conditional compilation provided
by the symlinks can be a convenient way to ad-hoc debug the core C++ code by printing (`std::cout`) intermediary values.
