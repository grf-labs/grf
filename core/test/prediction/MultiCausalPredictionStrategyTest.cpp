...
TEST_CASE("multi causal error estimates with one continuous treatment is identical to causal forest", "[multi_causal, prediction]") {
  std::unique_ptr<Data> data = load_data("test/forest/resources/causal_data.csv");
  data->set_outcome_index(10);
  data->set_treatment_index(0); // Set the treatment variable to the first continous covariate
  data->set_instrument_index(0);
  ...
  for (size_t i = 0; i < num_nodes; i++) {
    if (prediction_values.empty(i)) {
      ...
      auto prediction = prediction_strategy.compute_error(sample, prediction_values.get_values(i), prediction_values, *data);
      auto prediction_multi = multi_prediction_strategy.compute_error(sample, multi_prediction_values.get_values(i), multi_prediction_values, *data);
      REQUIRE(equal_doubles(prediction[0].first, prediction_multi[0].first, 1e-5));
      REQUIRE(equal_doubles(prediction[0].second, prediction_multi[0].second, 1e-5));
    }
  }
}
