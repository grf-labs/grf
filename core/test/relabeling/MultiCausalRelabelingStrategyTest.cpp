...
Eigen::ArrayXXd get_relabeled_outcomes(std::vector<double> observations,
                                       size_t num_samples,
  ...
  std::unique_ptr<RelabelingStrategy> relabeling_strategy(new MultiCausalRelabelingStrategy(num_treatments, influence_weight));
...
TEST_CASE("multi causal relabeling with optional weights works as expected", "[multi causal, relabeling]") {
  size_t num_samples = 10;
  size_t num_treatments = 2;
  std::vector<double> observations = {
    0.747, 0.207, 1.267, -0.503, -1.683, 3.547, 2.237, -5.123,
