...
/*
 * Aug 3, 2019 - Changes when porting to GRF:
 *  - No lines were deleted
 *  - Lines that contained *BOOST* or *mpl* were commented out
 *  - Substitution: "uniform_01<RealType>()" to "boost::random::uniform_real_distribution<RealType>()"
 */
...
//                BOOST_ASSERT(k < 10);
            ...
            IntType generate(URNG& urng) const
            {
                ...
                while(true) {
                    ...
                    RealType v = boost::random::uniform_real_distribution<RealType>()(urng);
                    ...
                    if(v >= _u._ptrd.v_r) {
                        ...
                        u = boost::random::uniform_real_distribution<RealType>()(urng) - 0.5;
                    } else {
                        u = v/_u._ptrd.v_r - 0.93;
                        u = ((u < 0)? -0.5 : 0.5) - u;
                        // v = uniform_01<RealType>()(urng) * _u._ptrd.v_r;
                        v =  boost::random::uniform_real_distribution<RealType>()(urng) * _u._ptrd.v_r;
                    }
            ...
            IntType invert(URNG& urng) const
            {
                ...
                RealType u = boost::random::uniform_real_distribution<RealType>()(urng);
                while(u > p) {
                    u = u - p;
                    ++x;
                    p = _mean * p / x;
