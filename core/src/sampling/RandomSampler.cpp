...
void RandomSampler::draw_simple(std::vector<size_t>& result,
  ...
  boost::random::uniform_int_distribution<size_t> unif_dist(0, max - 1 - skip.size());
...
void RandomSampler::draw_fisher_yates(std::vector<size_t>& result,
  ...
  for (size_t i = result.size() - 1; i > 0; --i) {
    boost::random::uniform_int_distribution<size_t> distribution(0, i);
...
void RandomSampler::draw_weighted(std::vector<size_t> &result,
    ...
    for (size_t i = 0; i < num_samples; ++i) {
        do {
            ...
            double_draw = unif(random_number_generator) * cdf.back();
...
size_t RandomSampler::sample_poisson(size_t mean) {
    boost::poisson_distribution<size_t> distribution(mean);
