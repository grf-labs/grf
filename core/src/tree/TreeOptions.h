...
public:
  ...
  /**
  * get_honesty(), get_honesty_fraction(), get_prune() are all related:
  ...
  * if prune is true, the resulting honest tree is pruned such that the
  * tree in the estimation sample does not contain any empty leafs. If false,
  * the resulting tree may contain empty leafs, which yield NaN upon prediction.
  */
  bool get_honesty() const;
  double get_honesty_fraction() const;
  bool get_prune() const;
