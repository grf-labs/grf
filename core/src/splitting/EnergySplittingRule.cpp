...
EnergySplittingRule::EnergySplittingRule(size_t max_num_unique_values,
                                                 double alpha,
...
EnergySplittingRule::~EnergySplittingRule() {
  if (counter != nullptr) {
    delete[] counter;
  }
  if (sums != nullptr) {
...
bool EnergySplittingRule::find_best_split(const Data& data,
                                              size_t node,
  ...
  // create the initial seg tree
  std::vector<double> yvalues;
  for (auto& sample : samples[node]) {
    yvalues.push_back(responses_by_sample[sample]);
  }
  
  segtree seg_tree(yvalues);
  ...
  for (auto& var : possible_split_vars) {
    ...
    find_best_split_value_large_q(data, node, var, size_node, min_child_size,
                                    best_value, best_var, best_decrease, responses_by_sample, samples, seg_tree);
...
void EnergySplittingRule::find_best_split_value_small_q(const Data& data,
...
void EnergySplittingRule::find_best_split_value_large_q(const Data& data,
                                                            size_t node,
                                                            size_t var,
                                                            //double sum_node,
                                                            size_t size_node,
  ...
  for (auto& sample : samples[node]) {
    ...
    seg_tree.insert_point(sums[index], 1);
    ++counter[index];
  }
  ...
  for (size_t i = 0; i < num_unique - 1; ++i) {
    ...
    // update the segtree
    if (counter[i] != 0) {
      for (size_t r = 1; r <= counter[i]; ++r) {
        seg_tree.remove_point(sums[i], 1);
        seg_tree.insert_point(sums[i], 0);
      } 
    }
