...
void RegressionSplittingRule::find_best_split_value_small_q(const Data& data,
  ...
  double* sums_right;
  double* weight_sums_right;
  size_t* n_right;
  sums_right = sums;
  weight_sums_right = weight_sums;
  n_right = counter;
  ...
  for (auto& sample : samples[node]) {
    ...
    for (size_t i = 0; i < num_splits; ++i) {
      if (value > possible_split_values[i]) {
        ...
        sums_right[i] += sample_weight * response;
      } else {
        break;
      }
    }
  ...
  for (size_t i = 0; i < num_splits; ++i) {
    ...
    double decrease = (sum_left * sum_left / weight_sum_left + sum_right * sum_right / weight_sum_right) * size_node / weight_sum_node;
