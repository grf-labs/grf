        ...
        while (level.size() > 0) {
            ...
            for (size_t node : level) {
                ...
                if (tree->is_leaf(child_nodes[0][node])==false) {
                    //offset the variable name by 1 to match in indices in R
                    variable_left = tree->get_split_vars().at(child_nodes[0][node])+1;
                }
                
                if (tree->is_leaf(child_nodes[1][node])==false) {
                    //offset the variable name by 1 to match in indices in R
                    variable_right = tree->get_split_vars().at(child_nodes[1][node])+1;
                }
